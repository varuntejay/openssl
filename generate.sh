...
function generateCSR () {
	...
	if [[ $org != "NULL" && $orgUnit != "NULL" ]] ; then
		openssl req -new -config $confFile -extensions v3_intermediate_ca -x509 -key $keyFile -out $csrFile -subj "/C=US/ST=California/L=San Francisco/CN=$CN/O=$org/OU=$orgUnit"
		RC=$?
	elif [[ $org != "NULL" ]] ; then
		openssl req -new -config $confFile -extensions v3_intermediate_ca -x509 -key $keyFile -out $csrFile -subj "/C=US/ST=California/L=San Francisco/CN=$CN/O=$org"
		RC=$?
	elif [[ $orgUnit != "NULL" ]] ; then
		openssl req -new -config $confFile -extensions v3_intermediate_ca -x509 -key $keyFile -out $csrFile -subj "/C=US/ST=California/L=San Francisco/CN=$CN/OU=$orgUnit"
		RC=$?
	else
		openssl req -new -config $confFile -extensions v3_intermediate_ca -x509 -key $keyFile -out $csrFile -subj "/C=US/ST=California/L=San Francisco/CN=$CN"
		RC=$?
	fi
...
function generateCert () {
	...
	openssl x509 -days 365 -CAcreateserial -CAserial ca.seq -in $csrFile -CA $rootCaCert -CAkey $rootCaKey -out $certFile
	if 	[[ $? -ne 0 ]] ; then
...
function createCerts () {
	...
	generateKey $keyFile
	generateCSR $keyFile $CN $signCertFile $signKeyFile $csrFile $caFlag $org $orgUnit
	generateCert $csrFile $signCertFile $signKeyFile $certFile 

	typeset ski=$(getSKI $certFile)
	mv $keyFile $dir/$ski"_sk"
	key=$dir/$ski"_sk"
...
function generateOrdererCerts () {
	...
	# newCaKey=$PWD/$(getSKI $newCaCert)"_sk"
	...
	# newTlscaKey=$PWD/$(getSKI $newTlscaCert)"_sk"
	...
	createCerts ca.$CN $signCert $signKey $caCsr $caCert $caKey $caDir "true" "NULL" "NULL"
	...
	createCerts tlsca.$CN $signCert $signKey $tlscaCsr $tlscaCert $tlscaKey $tlscaDir "true" "NULL" "NULL"
	...
	createCerts "Admin@$CN" $tlscaCert $tlscaKey $adminTlscaCsr $adminTlscaCert $adminTlscaKey $usersTlsDir "false" "NULL" "NULL"
	adminTlscaKey=$usersTlsDir/$(getSKI $adminTlscaCert)"_sk"
	...
	createCerts "Admin@$CN" $caCert $caKey $adminCaCsr $adminCaCert $adminCaKey $usersMspAdminDir "false" "NULL" "NULL" 
	adminCaKey=$usersMspAdminDir/$(getSKI $adminCaCert)"_sk"
	...
	createCerts "orderer.$CN" $tlscaCert $tlscaKey $ordererTlscaCsr $ordererTlscaCert $ordererTlscaKey $ordererMspTlscaDir "false" "NULL" "NULL"
	ordererTlscaKey=$ordererMspTlscaDir/$(getSKI $ordererTlscaCert)"_sk"
	...
	createCerts "orderer.$CN" $caCert $caKey $ordererCaCsr $ordererCaCert $ordererCaKey $ordererMspCaDir "false" "NULL" "NULL"
	ordererCaKey=$ordererMspCaDir/$(getSKI $ordererCaCert)"_sk"
...
function generatePeerCerts () {
	...
	createCerts "Admin@$CN" $caCert $caKey $adminCaCsr $adminCaCert $adminCaKey $usersMspAdminDir "false" "NULL" "client"
	adminCaKey=$usersMspAdminDir/$(getSKI $adminCaCert)"_sk"
	...
	do
		...
		createCerts "User$userNO@$CN" $tlscaCert $tlscaKey $userTlscaCsr $userTlscaCert $userTlscaKey $usersTlsDir "false" "NULL" "NULL"
		userTlscaKey=$usersTlsDir/$(getSKI $userTlscaCert)"_sk"
		...
		createCerts "User$userNO@$CN" $caCert $caKey $userCaCsr $userCaCert $userCaKey $usersMspAdminDir "false" "NULL" "client"
		userCaKey=$usersMspAdminDir/$(getSKI $userCaCert)"_sk"
	...
	do
		...
		createCerts "peer$peerNO.$CN" $tlscaCert $tlscaKey $peerTlscaCsr $peerTlscaCert $peerTlscaKey $peerMspTlscaDir "false" "NULL" "NULL"
		peerTlscaKey=$peerMspTlscaDir/$(getSKI $peerTlscaCert)"_sk"
		...
		createCerts "peer$peerNO.$CN" $caCert $caKey $peerCaCsr $peerCaCert $peerCaKey $peerMspCaDir "false" "NULL" "peer"
		peerCaKey=$peerMspCaDir/$(getSKI $peerCaCert)"_sk"
